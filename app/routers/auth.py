    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "message": "Dialpad session created successfully"
            }
        }
...
class StatusResponseData(BaseModel):
    authenticated: bool = Field(False, description="Dialpad API authentication status")
...
async def auth_status(
    ...
    """
    Get authentication status for Dialpad provider and optionally validate a session.

    Without session_id: Returns boolean flag for Dialpad authentication state.
    ...
    try:
        if session_id:
            ...
            try:
                provider_data = pm.get_provider(tenant_id, 'dialpad')
